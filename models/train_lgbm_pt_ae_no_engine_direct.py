...
def train_autoencoder(auto_model_base, train_loader, val_loader, epochs, lr, patience, model_save_path, model_name="Autoencoder"):
...
if __name__ == '__main__':
    ...
    for ae_type, prob_cols_subset, save_path, model_var_name in [
        ...
        if os.path.exists(save_path) and not FORCE_RETRAIN_AES:
            ...
            if seq_data_all_for_current_ae.shape[0] > AE_TOTAL_BATCH_SIZE:
                X_tr, X_val = train_test_split(seq_data_all_for_current_ae, test_size=AE_VALID_SPLIT, random_state=RANDOM_STATE)
                loader_tr = DataLoader(TensorDataset(torch.from_numpy(X_tr)), batch_size=AE_TOTAL_BATCH_SIZE, shuffle=True, num_workers=2, pin_memory=(DEVICE.type == 'cuda'))
                loader_val = DataLoader(TensorDataset(torch.from_numpy(X_val)), batch_size=AE_TOTAL_BATCH_SIZE, shuffle=False, num_workers=2, pin_memory=(DEVICE.type == 'cuda'))
                base_ae_model = ProbAutoencoder().to(DEVICE)
                current_ae_model = train_autoencoder(base_ae_model, loader_tr, loader_val, AE_EPOCHS, AE_LEARNING_RATE, AE_EARLY_STOPPING_PATIENCE, save_path, ae_type)
            else:
    ...
    fen_features_df = pd.DataFrame(combined_df['FEN'].progress_apply(get_extended_fen_features).tolist(), index=combined_df.index)
    ...
    if trained_rapid_ae or trained_blitz_ae:
        ...
        success_prob_df_subset = combined_df[prob_cols_all] if prob_cols_all else pd.DataFrame()  # Handle if prob_cols_all is empty
