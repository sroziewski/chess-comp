...
def add_pgn_notation(df, stockfish_df):
    ...
    if 'engine_top_move_uci' in result_df.columns:
        ...
        for idx, row in tqdm(result_df.iterrows(), total=len(result_df), desc="Converting top moves"):
    ...
    if 'engine_second_move_uci' in result_df.columns:
        ...
        for idx, row in tqdm(result_df.iterrows(), total=len(result_df), desc="Converting second moves"):
...
def main():
    ...
    try:
        ...
        logger.info(f"Loaded {len(original_train_df)} original training samples with {original_train_df.shape[1]} features")
        
        # Load original test data
        logger.info(f"Loading original test data from {args.original_test}")
        original_test_df = pd.read_csv(args.original_test)
        logger.info(f"Loaded {len(original_test_df)} original test samples with {original_test_df.shape[1]} features")
        ...
        logger.info(f"Loaded {len(stockfish_train_df)} Stockfish training samples with {stockfish_train_df.shape[1]} features")
        ...
        logger.info(f"Loaded {len(stockfish_test_df)} Stockfish test samples with {stockfish_test_df.shape[1]} features")
